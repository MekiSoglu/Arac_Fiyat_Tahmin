...
class PassatEnsembleRunner:
    # ====== yollar ======
    MODEL_DIR_LIGHT  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\passat_model_tahmin\light")
    MODEL_PATH_CAT   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\passat_model_tahmin\catboster\passat_residual_catboost.cbm")
    MODEL_PATH_RF    = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\passat_model_tahmin\randomForest\passat_residual_rf.joblib")
    MODEL_PATH_HGBR  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\passat_model_tahmin\stick-learn\passat_residual_hgbr.joblib")
    REF_PATH         = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\gundelik_passat_ilanlari\full-passat.xlsx")
    INPUT_PATH       = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\passat\gundelik_passat_ilanlari\passat_17eylül.xlsx")
    ...
    def parse_year_strict(x):
        ...
        v = PassatEnsembleRunner.to_number(x)
    ...
    def find_link_col(df):
        return (
            PassatEnsembleRunner.find_first(df, ["Link", "URL", "Url", "Bağlantı", "Baglanti"])
            or PassatEnsembleRunner.pick_col(df, ["link", "url", "bağlantı", "baglanti"])
        )
    ...
    def run(self):
        ...
        try:
            ...
            model_lgb_path = None
            for p in [self.MODEL_DIR_LIGHT / "light" / "passat_residual_lgbm.txt",
                      self.MODEL_DIR_LIGHT / "passat_residual_lgbm.txt"]:
            ...
            if model_lgb_path is None or not model_lgb_path.exists():
                raise FileNotFoundError("LightGBM model (passat_residual_lgbm.txt) bulunamadı.")
