...
OUT_DIR   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\randomF")
OUT_DIR.mkdir(parents=True, exist_ok=True)

HIT_LIMIT   = 100_000         # Hit@50k eşiği
...
def resolve_input_file(explicit: str) -> Path:
    ...
    - Ebeveyn klasör bulunamazsa iki olası isim: 'ilanlari' ve 'ilanları'
    """
    p = Path(explicit)
    ...
    # 2) Ebeveyn klasörü (gerekiyorsa alternatif klasör adları)
    parent = p.parent
...
def main():
    ...
    # Numeric dönüşümler + filtre
    df["_price"] = df[col_price].apply(to_number)
    df["_year"]  = df[col_year].apply(parse_year_strict)
    df["_km"]    = df[col_km].apply(parse_km)  # <<< KM artık güvenli parse

    mask = ((df["_year"] >= 2015) &
            (df["_price"] > 0) & (df["_price"] < 1e9) &
            (df["_km"].notna()) & (df["_km"] >= 0) & (df["_km"] < 2e6))  # <<< KM NaN hariç
    df = df[mask].copy()
    ...
    for i, (tr_idx, va_idx) in enumerate(kf.split(df), 1):
        ...
        # Preprocess + Model (OneHot dense + RF)
    ...
    pipe_full = Pipeline([("prep", pre_full), ("model", rf_full)])
    pipe_full.fit(X_full, y_full_resid)

    # Kaydet
    model_path = OUT_DIR / "corolla_residual_rf.joblib"
    joblib.dump({
        "pipeline": pipe_full,
        "num_cols": num_cols,
