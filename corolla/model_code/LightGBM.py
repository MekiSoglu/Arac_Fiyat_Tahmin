...
# Parametreler
# ==============================
FILE_NAME = r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\gundelik_corolla_ilanları\corolla_16eylül.xlsx"
OUT_DIR   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\light")
...
def resolve_input_file(explicit: str) -> Path:
    ...
    - Yoksa ebeveyn klasörde aksansız-ad karşılaştırmasıyla arar
    - Yine yoksa ebeveyn klasörde 'superb_*.xlsx' en yenisini alır
    """
    p = Path(explicit)
    ...
    # 2) Ebeveyn klasörü (yoksa tahmin edilen klasör)
    parent = p.parent
    if not parent.exists():
        # iki olası isim: 'ilanlari' ve 'ilanları'
...
def parse_km(x):
    ...
    s = s.replace("km", "")
    s = s.replace(" ", "")
    # KM'de kesir kullanılmadığı için nokta/virgül tamamen silinir
    s = s.replace(".", "").replace(",", "")
    s = re.sub(r"[^0-9]", "", s)
...
def avg_same_clean(df_ref, row, col_model, col_year, col_km, part_cols_map, km_window=30_000, min_n=3):
    """1) Aynı temizlik: Model aynı, Yıl aynı, KM ±30k, parça durumları birebir aynı."""
...
def avg_lenient(df_ref, row, col_model, col_year, col_km, km_window=50_000, year_window=1, min_n=3):
    """2) Esnek: Model aynı, Yıl ±1, KM ±50k, parça durumları serbest."""
    if col_model is None or col_year is None or col_km is None: return np.nan
    model = row.get(col_model, np.nan); year = row.get(col_year, np.nan); km = row.get(col_km, np.nan)
    if pd.isna(model) or pd.isna(year) or pd.isna(km): return np.nan
    m = ((df_ref[col_model] == model) &
...
def main():
    ...
    # Numeric + filtre (KM güvenli parse + NaN hariç)
    ...
    model_path = OUT_DIR / "corolla_residual_lgbm.txt"  # isim değişmeyecekse böyle bırakın
    ...
    # İnference için meta bilgileri kaydet
    meta = {
