...
class CorollaEnsembleRunner:
    # ====== yollar ======
    MODEL_DIR_LIGHT  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\light")
    MODEL_PATH_CAT   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\catboster\corolla_residual_catboost.cbm")
    MODEL_PATH_RF    = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\randomF\corolla_residual_rf.joblib")
    MODEL_PATH_HGBR  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\corolla_model_tahminleri\stic\corolla_residual_hgbr.joblib")
    REF_PATH         = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\corolla\gundelik_corolla_ilanları\corolla_16eylül.xlsx")
    INPUT_PATH       = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\gundelik_golf_ilanları\golf_17eylül.xlsx")
    ...
    def parse_year_strict(x):
        ...
        v = CorollaEnsembleRunner.to_number(x)
    ...
    def find_link_col(df):
        return (
            CorollaEnsembleRunner.find_first(df, ["Link", "URL", "Url", "Bağlantı", "Baglanti"])
            or CorollaEnsembleRunner.pick_col(df, ["link", "url", "bağlantı", "baglanti"])
        )
    ...
    def run(self):
        ...
        try:
            ...
            model_lgb_path = None
            for p in [self.MODEL_DIR_LIGHT / "light" / "corolla_residual_lgbm.txt",
                      self.MODEL_DIR_LIGHT / "corolla_residual_lgbm.txt"]:
            ...
            if model_lgb_path is None or not model_lgb_path.exists():
                raise FileNotFoundError("LightGBM model (corolla_residual_lgbm.txt) bulunamadı.")
