...
class GolfEnsembleRunner:
    # ====== yollar ======
    MODEL_DIR_LIGHT  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\golf_model_tahminleri\light")
    MODEL_PATH_CAT   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\golf_model_tahminleri\catboster\golf_residual_catboost.cbm")
    MODEL_PATH_RF    = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\golf_model_tahminleri\randomF\golf_residual_rf.joblib")
    MODEL_PATH_HGBR  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\golf_model_tahminleri\stic\golf_residual_hgbr.joblib")
    REF_PATH         = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\golf\gundelik_golf_ilanları\golf_13eylül.xlsx")
    ...
    def parse_year_strict(x):
        ...
        v = GolfEnsembleRunner.to_number(x)
    ...
    def find_link_col(df):
        return (
            GolfEnsembleRunner.find_first(df, ["Link", "URL", "Url", "Bağlantı", "Baglanti"])
            or GolfEnsembleRunner.pick_col(df, ["link", "url", "bağlantı", "baglanti"])
        )
    ...
    def run(self):
        ...
        try:
            ...
            model_lgb_path = None
            for p in [self.MODEL_DIR_LIGHT / "light" / "golf_residual_lgbm.txt",
                      self.MODEL_DIR_LIGHT / "golf_residual_lgbm.txt"]:
            ...
            if model_lgb_path is None or not model_lgb_path.exists():
                raise FileNotFoundError("LightGBM model (golf_residual_lgbm.txt) bulunamadı.")
