...
class SuperbEnsembleRunner:
    # ====== yollar ======
    MODEL_DIR_LIGHT  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\superb_model_tahminleri\light")
    MODEL_PATH_CAT   = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\superb_model_tahminleri\catboster\passat_residual_catboost.cbm")
    MODEL_PATH_RF    = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\superb_model_tahminleri\randomF\passat_residual_rf.joblib")
    MODEL_PATH_HGBR  = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\superb_model_tahminleri\stick\passat_residual_hgbr.joblib")
    REF_PATH         = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\gündelik_superb_ilanlari\superb_12eylül.xlsx")
    INPUT_PATH       = Path(r"C:\Users\EXCALIBUR\Desktop\sahibinden\superb\gündelik_superb_ilanlari\superb_17eylül.xlsx")
    ...
    def parse_year_strict(x):
        ...
        v = SuperbEnsembleRunner.to_number(x)
    ...
    def find_link_col(df):
        return (
            SuperbEnsembleRunner.find_first(df, ["Link", "URL", "Url", "Bağlantı", "Baglanti"])
            or SuperbEnsembleRunner.pick_col(df, ["link", "url", "bağlantı", "baglanti"])
        )
    ...
    def run(self):
        ...
        try:
            ...
            model_lgb_path = None
            for p in [self.MODEL_DIR_LIGHT / "light" / "superb_residual_lgbm.txt",
                      self.MODEL_DIR_LIGHT / "superb_residual_lgbm.txt"]:
            ...
            if model_lgb_path is None or not model_lgb_path.exists():
                raise FileNotFoundError("LightGBM model (superb_residual_lgbm.txt) bulunamadı.")
